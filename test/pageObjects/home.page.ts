...
class HomePage extends Page {
    get lblDiscount () { return $('h2[data-test-discount]')}

    get lblCategory () { return $(`#challenge-2-category`)}

    get lblGiftCode () { return $(`h6[data-test-gift-code]`)}
    ...
    async takeTheChallengeDiscount () {
        ...
        let iDiscount = await this.getNumberValue(this.lblDiscount)
    ...
    async takeTheChallengeRevealTheDeal () {
        ...
        await uiTextbox(`Scroll down to Deal of The Week, click on REVEAL THE DEAL, copy the code, then paste it here.`).setValue(strGiftCode)
        await uiButton('CHECK').click()
    }
